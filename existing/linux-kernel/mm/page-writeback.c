 ...
 */
//KTDEFEX(balance_io_wait);
/* ych	*/
KTDEFEX(balance_dirty_pages_calc);
/* ych	*/
static void balance_dirty_pages_internal(struct bdi_writeback *wb,
...
//		ktget(&stopwatch[1]);
//		ktput(stopwatch, balance_io_wait);
				...
				unsigned long pages_dirtied)
{
	__maybe_unused ktime_t stopwatch[2];
	ktget(&stopwatch[0]);
	balance_dirty_pages_internal(wb, pages_dirtied);
	ktget(&stopwatch[1]);
	ktput(stopwatch, balance_dirty_pages_calc);
}
/* ych	*/
