...
KTDEFEX(avail_grant_wait);
KTDEFEX(dirty_page_wait);
atomic_t first_call;
static int osc_enter_cache(const struct lu_env *env, struct client_obd *cli,
			   struct osc_async_page *oap, int bytes)
{
	__maybe_unused ktime_t stopwatch[2];
	...
	if (cli->cl_avail_grant < bytes) {
		...
		ktget(&stopwatch[1]);
		ktput(stopwatch, avail_grant_wait);
	} else {
		ktget(&stopwatch[0]);
		remain = wait_event_idle_exclusive_timeout_cmd(
			cli->cl_cache_waiters,
			...
			cli_lock_after_unplug(cli));
		ktget(&stopwatch[1]);
		ktput(stopwatch, dirty_page_wait);
		   ...
		   struct osc_object *osc, int async)
{
	...
	if (!async) {
		spin_lock(&cli->cl_loi_list_lock);
//		if (atomic_read(&cli->my_flag))
//			printk("[%s] sync - grant=%lu\n", __func__, cli->cl_avail_grant);
...
//	cli->cl_avail_grant = (unsigned long)500 * 1024 * 1024 * 1024;
	/* ych	*/
