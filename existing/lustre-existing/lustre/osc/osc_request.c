...
static int osc_shrink_grant(struct client_obd *cli)
{
	...
	// cl_max_rpcs_in_flight = 32, cl_max_pages_per_rpc = 1024, PAGE_SHIFT = 12
	// target_bytes = 132MB
...
void osc_init_grant(struct client_obd *cli, struct obd_connect_data *ocd)
{
	...
	//cli->cl_avail_grant = 16UL * 1024 * 1024 * 1024;
	//cli->cl_avail_grant = 256UL * 1024 * 1024 * 1024;
	//cli->cl_dirty_max_pages = 128UL * 1024 * 1024 * 1024;   
	//cli->cl_dirty_max_pages = 256UL * 1024 * 1024 * 1024;   
	//cli->cl_dirty_max_pages = 256000UL * 256;     // 256GB
	//printk("[%s] cl_avail_grant=%lu, cl_dirty_max_pages=%lu\n", __func__,
	//		cli->cl_avail_grant, cli->cl_dirty_max_pages);
...
//	printk("[%s] pga[0]->off=%lld, count=%d\n", __func__,
//			pga[0]->off,
//			pga[0]->count);
...
no_bulk:
	...
	/* ych	*/
//	if (desc != NULL) {
//		printk("[%s] RPC payload: bd_nob=%u bytes\n", __func__, desc->bd_nob);
//	}
	/* ych	*/
	...
	/* ych	*/
	//printk("[%s] requested_nob = %u\n", __func__, requested_nob);
	/* ych	*/
...
/* ych	*/
//static unsigned long long max_grant_seen = 0;
/* ych	*/
...
//	if (cli->cl_avail_grant > (unsigned long)230 * 1024 * 1024 * 1024) {
//		printk("[%s] body->oa.o_grant = %lld\n", __func__, body->oa.o_grant);
//		printk("[%s] cli->cl_avail_grant = %ld\n", __func__, cli->cl_avail_grant);
//		printk("[%s] cli->cl_dirty_grant = %ld\n", __func__, cli->cl_dirty_grant);
//	}
...
//	if (cli->cl_avail_grant > max_grant_seen) {
//		max_grant_seen = cli->cl_avail_grant;
//
//		printk("max_grant_seen = %llu\n", max_grant_seen);
//		printk("o_grant = %llu\n", body->oa.o_grant);
//	}
